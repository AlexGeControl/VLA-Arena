...
VLA-Arena 是一个开源的基准测试平台，用于系统评测视觉-语言-动作（VLA）模型。VLA-Arena 提供完整的工具链，涵盖**场景建模**、**行为收集**、**模型训练**和**评测**。它具有13个专业套件中的150+个任务、分层难度级别（L0-L2），以及用于安全性、泛化性和效率评测的综合指标。
...
- **🚀 端到端即开即用**：我们提供完整统一的工具链，涵盖从场景建模和行为收集到模型训练和评测的所有内容。配合全面的文档和教程，您可以在几分钟内开始使用。

- **🔌 即插即用评测**：无缝集成和基准测试您自己的VLA模型。我们的框架采用统一API设计，使新架构的评测变得简单，只需最少的代码更改。

- **🛠️ 轻松任务定制**：利用行为域定义语言（BDDL）快速定义全新的任务和安全约束。其声明性特性使您能够以最少的努力实现全面的场景覆盖。
