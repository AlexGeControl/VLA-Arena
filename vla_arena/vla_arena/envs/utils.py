...
class SmoothWaypointMotionGenerator:
    def __init__(self, waypoints, start_quat, segment_time=10, dt=0.01, loop=False):   
        ...
        self.positions = [list(map(float,wp[0])) for wp in waypoints]  # Extract position information
        self.quaternions = [direction_to_quaternion(list(map(float,wp[1]))) for wp in waypoints]  # Extract orientation information
        
        self.n_segments = len(waypoints) - 1
        
        self.segment_time = float(segment_time)
        self.dt = float(dt)
        self.steps_per_segment = max(1, int(self.segment_time / self.dt))  # Convert to integer steps
        self.loop = bool(loop)  
