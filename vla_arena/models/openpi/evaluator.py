...
def run_task(
    ...
    env, task_description = get_vla_arena_env(task, resolution=VLA_ARENA_ENV_RESOLUTION, add_noise=cfg.add_noise,camera_offset=cfg.camera_offset,adjust_light=cfg.adjust_light,randomize_color=cfg.randomize_color)
    # print(task.language)
...
def get_vla_arena_env(task, resolution=256,add_noise=False,randomize_color=False,adjust_light=False,camera_offset=False):
...
def main(cfg=None):
    ...
    if isinstance(cfg, (str, pathlib.Path)):
        ...
        raise ValueError(f"Unsupported config type: {type(cfg)}. Expected GenerateConfig, str, Path, or None.")
