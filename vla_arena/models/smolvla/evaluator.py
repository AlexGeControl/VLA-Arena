...
def eval_vla_arena(args: Args) -> None:
    ...
    for task_id in tqdm(range(num_tasks_in_suite), desc="Tasks"):
        ...
        env, task_description = _get_vla_arena_env(task, LIBERO_ENV_RESOLUTION, args.seed,args.add_noise,args.randomize_color,args.adjust_light,args.camera_offset)
        ...
        for episode_idx in tqdm(
            ...
            if should_save_video:
                ...
                video_path = (
                    Path(video_out_path) / f"{TIME}_rollout_task_{task_id}_episode_{episode_idx}_{task_segment}_{suffix}.mp4"
                )
                fps = 30
...
def _get_vla_arena_env(task, resolution, seed,add_noise=False,randomize_color=False,adjust_light=False,camera_offset=False):
    """Initializes and returns the LIBERO environment, along with the task description."""
    task_description = task.language
    task_bddl_file = Path(get_vla_arena_path("bddl_files")) / task.problem_folder / f"level_{task.level}" / task.bddl_file
    env_args = {
        "bddl_file_name": str(task_bddl_file),
        "camera_heights": resolution,
        "camera_widths": resolution,
        "camera_offset": camera_offset,"color_randomize": randomize_color,"add_noise": add_noise, "light_adjustment":adjust_light,
    }
...
def main(cfg: Union[Args, str, Path]):
