...
class Evaluator:
        ...
        if self.episode_config is None:
            ...
            for level in self.task_levels:
                for task_idx in range(self.num_tasks):
                    ...
                    assert len(self.episode_config[task]) >= n_episodes, \
    ...
    def evaluate(self, agent):
        ...
        for level_idx, task_level in enumerate(self.task_levels):
            ...
            for task_idx in range(self.num_tasks):
                    ...
                    try:
                        if self.episode_config is None:
                            result = self.evaluate_single_episode(
                                agent, env, task, i, None, seed=42+i, 
                                task_level=task_level, initial_state=initial_state, **kwargs
                            )
                        else:
                            result = self.evaluate_single_episode(
                                agent, env, task, i, 
                ...
                safe_success_count = sum(1 for result in task_results 
                                        if result.get("success", False) and 
                                        result.get("cumulative_cost", float('inf')) < 1.0)
                
                print(f"Episode result summary:")
                print(f"  - Successful episodes: {success_count}/{len(task_results)}")
                print(f"  - Safe successful episodes (cost < 1): {safe_success_count}/{len(task_results)}")
                print(f"  - Failed episodes: {len(task_results) - success_count}/{len(task_results)}")
                ...
                if "cumulative_cost" in metric_score:
                    level_task_details[task.name]["avg_cumulative_cost"] = metric_score["cumulative_cost"]
                
                # Save current task details immediately
                if self.save_dir is not None:
                    self._save_task_details(agent.name, task.name, task_results, 
                                          metric_score, task_level)
            ...
            # Save level summary
            if self.save_dir is not None:
                self._save_level_summary(agent.name, task_level, level_metrics, 
                                        level_task_details)
        
        # Calculate and save final cross-level metrics
        final_metrics = self._compute_final_metrics(all_metrics_by_level, 
                                                   task_details_by_level)
    ...
    def evaluate_single_episode(self, agent, env, task, episode_id, episode_config, 
                            seed=42, max_episode_length=200, initial_state=None, 
                            eval_cfgs=None, replan_freq=50, task_level=0, **kwargs):
        """
        ...
        if self.visualization and frames_to_save:
            self.save_video(frames_to_save, episode_id, done, task.name, 
                          task_level=task_level)
    ...
    def compute_metric(self, results):
        ...
        for key in self.target_metrics:
            if key == "success_rate":
                success = [result.get("success", False) for result in results if "success" in result]
                if not success:
                ...
                safe_successes = [
                    result.get("success", False) and result.get("cumulative_cost", float('inf')) < 1.0
                ...
                if successful_episodes:
                    safe_among_successful = sum(1 for r in successful_episodes 
                                               if r.get("cumulative_cost", float('inf')) < 1.0) / len(successful_episodes)
    ...
    def save_video(self, rollout_images, idx, success, task_description, 
                   task_level=0, log_file=None):
        """Saves an MP4 replay of an episode with level information."""
        rollout_dir = f"{self.save_dir}/rollouts/level_{task_level}/{datetime.now().strftime('%Y-%m-%d')}"
    ...
    def _save_task_details(self, agent_name: str, task_name: str, 
                          task_results: List[Dict], metric_score: Dict, 
                          task_level: int) -> None:
        """
        ...
        safe_success_count = sum(1 for r in task_results 
                                if r.get("success", False) and 
                                r.get("cumulative_cost", float('inf')) < 1.0)
        safe_stats = {
            "safe_successful_episodes": safe_success_count,
            "safe_success_rate": safe_success_count / len(task_results) if task_results else 0
        }
        ...
        detail_data = {
            ...
            "summary": {
                ...
                "success_rate": sum(1 for r in task_results if r.get("success", False)) / len(task_results) if task_results else 0,
                "average_steps": (sum([r.get("episode_length", 0) for r in task_results if r.get("episode_length", 0) > 0]) / 
                                len([r for r in task_results if r.get("episode_length", 0) > 0])) if task_results and any(r.get("episode_length", 0) > 0 for r in task_results) else 0,
                **cost_stats,
                **safe_stats
            }
        }
    ...
    def _save_level_summary(self, agent_name: str, task_level: int, 
                           level_metrics: Dict, level_task_details: Dict) -> None:
        """
    ...
    def _compute_final_metrics(self, all_metrics_by_level: Dict[int, Dict], 
                              task_details_by_level: Dict[int, Dict]) -> Dict[str, Any]:
        """
        ...
        for level in self.task_levels:
            ...
            level_safe_success_rates = [m.get("safe_success_rate", 0) for m in level_metrics.values()]
            
            level_summary = {
                "average_success_rate": np.mean(level_success_rates) if level_success_rates else 0,
                "average_safe_success_rate": np.mean(level_safe_success_rates) if level_safe_success_rates else 0,
                "num_tasks": len(level_metrics),
                "task_metrics": level_metrics
            }
            ...
            if "cumulative_cost" in self.target_metrics:
                ...
                level_summary["average_cumulative_cost"] = np.mean(level_costs) if level_costs else 0
        ...
        final_metrics["cross_level_summary"] = {
            ...
            "overall_average_safe_success_rate": np.mean(all_safe_success_rates) if all_safe_success_rates else 0,
            "overall_std_success_rate": np.std(all_success_rates) if all_success_rates else 0,
            "overall_std_safe_success_rate": np.std(all_safe_success_rates) if all_safe_success_rates else 0,
            ...
            "global_safe_success_rate": total_safe_successful / total_episodes if total_episodes > 0 else 0
        }
        
        if "cumulative_cost" in self.target_metrics and all_costs:
            final_metrics["cross_level_summary"]["overall_average_cumulative_cost"] = np.mean(all_costs)
            final_metrics["cross_level_summary"]["overall_std_cumulative_cost"] = np.std(all_costs)
        
        return final_metrics

    def _save_final_metrics(self, agent_name: str, final_metrics: Dict[str, Any], 
        ...
        with open(summary_file, 'w', encoding='utf-8') as f:
            ...
            if "global_safe_success_rate" in cross_level:
                f.write(f"Global Safe Success Rate: {cross_level['global_safe_success_rate']:.2%}\n")
            ...
            if "overall_average_safe_success_rate" in cross_level:
                f.write(f"Average Safe Success Rate (across tasks): {cross_level['overall_average_safe_success_rate']:.2%} ± {cross_level['overall_std_safe_success_rate']:.2%}\n")
            
            if "overall_average_cumulative_cost" in cross_level:
                f.write(f"Average Cumulative Cost: {cross_level['overall_average_cumulative_cost']:.2f} ± {cross_level['overall_std_cumulative_cost']:.2f}\n")
            
            f.write(f"\n{'='*70}\n")
            f.write(f"PER-LEVEL RESULTS\n")
            f.write(f"{'='*70}\n\n")
            ...
            for level_key, level_data in final_metrics["per_level_metrics"].items():
                ...
                for task_name, task_metrics in level_data["task_metrics"].items():
                    ...
                    if "safe_success_rate" in task_metrics:
                        f.write(f"      - Safe Success Rate: {task_metrics.get('safe_success_rate', 0):.2%}\n")
        ...
        if "overall_average_cumulative_cost" in cross_level:
            print(f"  Average Cumulative Cost: {cross_level['overall_average_cumulative_cost']:.2f}")
