...
CONFIG_NAME=""
EXP_NAME=""
MODEL_TYPE="pi0"  # pi0 or pi0_fast
...
def add_training_config():
    ...
    # Build wandb configuration
    wandb_config = ""
    if "$WANDB_ENABLED" == "false":
        wandb_config = "wandb_enabled=False,"
    ...
    pattern = r'(\s+)(\*roboarena_config\.get_roboarena_configs\(\),\s*\]\s*$)'
