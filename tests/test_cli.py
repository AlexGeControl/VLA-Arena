...
try:
    from vla_arena.cli import main as cli_main_module, eval, train
...
class TestCLIMain:
    ...
    def test_main_train_parser(self, monkeypatch):
        ...
        with patch('sys.argv', ['vla-arena', 'train', '--model', 'openvla', '--config', 'test.yaml']):
            try:
                cli_main_function()
            except SystemExit:
                pass
    ...
    def test_main_eval_parser(self, monkeypatch):
        ...
        with patch('sys.argv', ['vla-arena', 'eval', '--model', 'openvla', '--config', 'test.yaml']):
            try:
                cli_main_function()
            except SystemExit:
                pass
